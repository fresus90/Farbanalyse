...
export function goToScreen(id){
  ...
  if((id==='obQuiz'||id==='obResult')&&obPhotoDataUrl) showObPhotoPreview(obPhotoDataUrl);
}
...
export function showResult(){
  ...
  if(obPhotoDataUrl) showObPhotoPreview(obPhotoDataUrl);
}
...
export function applyQuizResult(typeKey){
  try{
    ...
    if(obPhotoDataUrl){
      originalDataUrl=obPhotoDataUrl;
      ...
      img.onload=function(){
        ...
        }).catch(function(){showInView(obPhotoDataUrl);});
      };
      img.onerror=function(){showInView(obPhotoDataUrl);};
      img.src=obPhotoDataUrl;
    }
  }catch(e){
    var qs=document.getElementById('quizScreen'); if(qs) qs.style.display='none';
    var vm=document.getElementById('viewMode');   if(vm) vm.style.display='block';
