...
export function captureObPhoto(){
  ...
  if(camFacing==='user'){ctx.translate(wc.width,0);ctx.scale(-1,1);}
  ctx.drawImage(video,0,0);
  obPhotoDataUrl=wc.toDataURL('image/jpeg',0.92);
  closeLiveCamera();showObPhotoPreview(obPhotoDataUrl);
}
...
export function handleObPhoto(input){
  ...
  var fr=new FileReader();
  fr.onload=function(e){obPhotoDataUrl=e.target.result;showObPhotoPreview(obPhotoDataUrl);};
  fr.readAsDataURL(file);
}
